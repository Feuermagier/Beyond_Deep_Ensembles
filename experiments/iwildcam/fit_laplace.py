...
def run(device, config, out_path, log, rep):
    ...
    # model = get_model("laplace_base", config, device)
    # model.load_state_dict(torch.load(f"results/MAP/log/rep_0{rep}map_final.tar"))
    ...
    # laplace = LaplaceApprox(model.models[0], regression=False, out_activation=torch.log, hessian="kron")
    # laplace.fit(LoaderMock())

    # print("Optimizing prior prec...")
    # laplace.optimize_prior_prec()

    # model.models[0] = laplace
...
def load_all(log, config, device):
    ...
    for i in range(3):
        single_model = get_model(config["model"], single_config, device)
        path = f"./results/{config['run_name']}_more/log/rep_0{i}map_final.tar"
        single_model.load_state_dict(torch.load(path))
        models_and_optimizer.append(single_model.models_and_optimizers[0])
...
def eval_laplace(model, config, log, device):
    ...
    test_results = eval_model(model, config, device, split="test", subsample=config["test_subsample"], multisample=False)
    log.info(f"Test: {test_results}")

    val_results = eval_model(model, config, device, split="val", subsample=config["test_subsample"], multisample=False)
    log.info(f"OOD Validation: {val_results}")

    id_val_results = eval_model(model, config, device, split="id_val", subsample=config["test_subsample"], multisample=False)
