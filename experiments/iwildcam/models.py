...
from src.algos.bbb_layers import BBBLinear, make_module_bbb
...
def build_map(config, device):
    model = get_base_model(nn.Linear(RESNET_OUT_DIMS, N_CLASSES), config).to(device)
...
def build_laplace_base(config, device):
    model = resnet50(pretrained=not config.get("scratch", False))
    model.fc = nn.Linear(RESNET_OUT_DIMS, N_CLASSES)
...
def build_mcd(config, device):
    head = nn.Sequential(
        ...
        nn.Linear(RESNET_OUT_DIMS, N_CLASSES)
    )
    model = get_base_model(head, config).to(device)
    optimizer = MAPOptimizer(model.parameters(), torch.optim.Adam(model.parameters(), **config["base_optimizer"]))
    return model, optimizer
...
def build_ll_swag(config, device):
    model = get_base_model(nn.Linear(RESNET_OUT_DIMS, N_CLASSES), config).to(device)
    ll_optimizer = SwagOptimizer(params=model[0].fc.parameters(), base_optimizer=torch.optim.Adam(model[0].fc.parameters(), **config["base_optimizer"]), **config["swag"])
    det_optimizer = torch.optim.Adam(itertools.chain(
        model[0].conv1.parameters(), 
        model[0].bn1.parameters(), 
        model[0].layer1.parameters(), 
        model[0].layer2.parameters(), 
        model[0].layer3.parameters(), 
        model[0].layer4.parameters()), **config["deterministic_optimizer"])
...
def build_bbb(config, device):
    ...
    model = get_base_model(BBBLinear(RESNET_OUT_DIMS, N_CLASSES, prior, prior), config).to(device)

    if config.get("all_layers_bbb", False):
        make_module_bbb(model, prior)
...
def build_rank1(config, device):
    prior = GaussianPrior(0, config["prior_std"])
    model = get_base_model(Rank1Linear(RESNET_OUT_DIMS, N_CLASSES, prior, components=config["rank1"]["components"]), config).to(device)
...
def build_ll_svgd(config, device):
    ...
    def reset_model():
        reset_model_params(model[0].fc)
    ll_optimizer = SVGDOptimizer(model[0].fc.parameters(), reset_model, torch.optim.Adam(model[0].fc.parameters(), **config["base_optimizer"]), **config["svgd"])
    det_optimizer = torch.optim.Adam(itertools.chain(
        model[0].conv1.parameters(), 
        model[0].bn1.parameters(), 
        model[0].layer1.parameters(), 
...
def build_ll_ivon(config, device):
    model = get_base_model(nn.Linear(RESNET_OUT_DIMS, N_CLASSES), config).to(device)
    ll_optimizer = iVONOptimizer(model[0].fc.parameters(), **config["ivon"])
    det_optimizer = torch.optim.Adam(itertools.chain(
        model[0].conv1.parameters(), 
        model[0].bn1.parameters(), 
        model[0].layer1.parameters(), 
...
def build_sngp(config, device):
    model = get_base_model(nn.Identity(), config).to(device)
...
def get_base_model(classification_head, config):
    model = resnet50(pretrained=not config.get("scratch", False))
