...
def run(device, config, out_path, log, rep):
    ...
    if config.get("eval_single", False):
        laplace_models = [fit_laplace(all_models_and_optimizers[0][0], trainloader, config, log, device)]
        eval_rep_single(0, laplace_models, single_model, config, log, device)
    else:
        laplace_models = []
        for i in range(6):
...
def fit_laplace(model, trainloader, config, log, device):
    ...
    if config.get("optimizer_prior_prec", True):
...
def eval_laplace(model, config, log, device):
    ...
    val_results = eval_model(model, config, device, split="val", subsample=config["test_subsample"], multisample=True)
    log.info(f"OOD Validation: {val_results}")

    id_val_results = eval_model(model, config, device, split="id_val", subsample=config["test_subsample"], multisample=True)
