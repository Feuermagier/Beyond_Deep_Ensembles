...
def get_region(metadata, region):
    return (metadata[:,0] == region).nonzero()

def eval_region(region, errors, confidences, lls, config):
    accuracy = errors[region].sum() / len(region)
    calibration = ClassificationCalibrationResults(config["ece_bins"], errors[region], confidences[region])
    return {
        "accuracy": accuracy,
        "log_likelihood": lls[region].mean(),
        "ece": calibration.ece,
...
def eval_model(model, config, device, split="test", subsample=None, multisample=False):
    ...
    dataset = wilds1.fmow_split(config["data_path"], split)
    loader = wilds1.fmow_loader(dataset, config["batch_size"], subsample=subsample)
    ...
    results = {
        "wilds": wilds_results,
        "worst_region_acc": wilds_results[0]["acc_worst_region"],
    }
    ...
    for region in range(5):
        region_indices = (metadata[:,0] == region).nonzero()
        results[f"region_{region}"] = eval_region(region_indices, errors, confidences, log_likelihoods, config)
...
def run(device, config, out_path, log, rep):
    ...
    name = config['model'] + (f"_p{config['ll_dropout_p']}" if 'll_dropout_p' in config else "")
    wandb.init(
        name=f"{name}-{config['members']}-({rep})", 
        project="fmow", 
        group=config["model"],
        config=config,
        tags=wandb_tags,
...
def train_model(ensemble: DeepEnsemble, device, config, log, out_path, start_epoch=0):
    ...
    for model_idx, (model, optimizer) in enumerate(ensemble.models_and_optimizers):
        ...
        if isinstance(optimizer, LastLayerBayesianOptimizer):
            scheduler = torch.optim.lr_scheduler.ExponentialLR(optimizer.ll_bayesian_optimizer.get_base_optimizer(), gamma=config["lr_decay"])
            scheduler2 = torch.optim.lr_scheduler.ExponentialLR(optimizer.deterministic_optimizer, gamma=config["lr_decay"])
        else:
        ...
        if config["train_on_val"]:
            ...
            trainloader = wilds1.fmow_loader(wilds1.iwfmowildcam_split(config["data_path"], "id_val"), config["batch_size"], subsample=config["subsample"])
        else:
            trainloader = wilds1.fmow_loader(wilds1.fmow_split(config["data_path"], "train"), config["batch_size"], subsample=config["subsample"])
