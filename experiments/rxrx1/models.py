...
def build_map(config, device):
    model = get_base_model(nn.Linear(RESNET_OUT_DIMS, N_CLASSES)).to(device)
...
def build_bbb(config, device):
    ...
    model = get_base_model(BBBLinear(RESNET_OUT_DIMS, N_CLASSES, prior, prior)).to(device)
...
def build_rank1(config, device):
    prior = GaussianPrior(0, config["prior_std"])
    model = get_base_model(Rank1Linear(RESNET_OUT_DIMS, N_CLASSES, prior, components=config["rank1"]["components"])).to(device)
...
def build_sngp(config, device):
    model = get_base_model(nn.Identity()).to(device)
